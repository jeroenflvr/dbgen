...
def export_tables_to_parquet(conn, tables, step, prefix, bucket):
    """
    ...
    for table in tables:
        ...
        # parquet_file = os.path.join(output_dir, table, f"{table}_{str(step).zfill(7)}.parquet")
        parquet_file = (
            f"s3://{bucket}/{prefix}/{table}/{table}_{str(step).zfill(7)}.parquet"
        )
        try:
...
def main():
    ...
    for step in range(STEP_START + 1000, CHILDREN):
        ...
        export_tables_to_parquet(conn, new_tables, step, OUTPUT_PREFIX, BUCKET)
